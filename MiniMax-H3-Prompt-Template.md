...
> **重要：只借人物、不借場景。** 若只參考「圖片裡的人物」，場景、背景、光線、機位要寫在提示詞裡——不要留白或暗示沿用參考圖的環境。Bot 會在每支 Ref2VA 片頭自動注入「只鎖人物外貌、不帶走參考圖場景/背景/光線/機位」的規則；但若你的提示詞本身沒寫場景，模型仍可能回退到參考圖的場景。要完全避免整段跳到參考圖場景，務必把目標場景寫進提示詞。
...
| 對白語言 | 提示詞含中文但沒指定語言時，Bot 自動鎖**普通話**；要粵語／其他口音就明寫（"all dialogue in Cantonese"） |
...
## 十一、Motion Context 長片接續（實驗性，Bot 預設）

長片超過 8 秒時，Bot 會把每幕拆成 ≤8 秒鏡頭逐鏡生成。預設接續模式是 **H3 Motion Context**（環境變數 `MINIMAX_H3_LONG_CONTINUITY=motion_context`）：每完成一鏡，Bot 會保存該鏡的 **AV latent**（影像 + 音訊），下一鏡生成時把上一鏡的**最後 22 幀 + 最後整整 1 秒音訊**固定在新一鏡的開頭、採樣後再裁掉。等於模型「親眼看到」上一鏡的收尾畫面、也「聽到」上一鏡的收尾音訊，直接從那裡接下去，而不是重來一次。

因為模型已經看到收尾畫面與音訊，**提示詞只要寫「繼續」，不要重寫收尾**：

1. **不要重排鏡頭、不要重述收尾** — 收尾幀與最後 1 秒音訊已被固定在新片開頭，會裁掉。寫「回到上一鏡的畫面」「重現剛纔的鏡頭」都會和固定幀打架。
2. **開頭 ~2 秒要「持住 + 微動」** — 固定幀比採樣長度短 22 幀（≈0.92 秒），新片開頭是「空」。寫成靜止畫面會變成**凍住**。要寫入呼吸、重心移動、視線變化——鏡頭可以定，角色不能定。
3. **畫面裡的人數／構圖要和收尾幀完全一致** — H3 把互相矛盾的構圖當成**並集**渲染（不是替換）。收尾是 A 的特寫，新片開頭卻寫「B 與 C 的雙人像」，畫面會三者同時出現。要換鏡頭，先用 ~2 秒**空隔（airlock）**持住上一鏡的收尾構圖、無對白，再切到新構圖。
4. **音訊是「接續」不是「參考」** — 最後 1 秒音訊已固定；提示詞只說「維持同一音樂底、同一速度、同一配樂，繼續生成這鏡剩下的音訊」。**不要**寫「参考 <Audio 1>」那種引用語（那是穩定尾幀模式才用）。
5. **時間軸要「預留固定頭」** — 22 幀 = 0.92 秒被裁掉。你寫的 4.0 秒節拍，實際上會落在檔案的 3.08 秒。長鏡頭的時間標記要記得這個偏移。

Bot 送出的長片提示詞已自動帶入上述 1–5 的接續規則（`continuity_instruction` 的 motion_context 分支），你只需按上面「六、長片時間軸」寫 GLOBAL + 各幕動作即可，**不要**自己再寫「回到上一鏡」「重放」之類的話。

### 已知的接續限制（README 原文摘錄）

- **接續鏈越長，音訊越糊** — 每鏡都是「上一鏡輸出再喂回去」，损失像影印影印本，高頻最先消失。影像比音訊撐得久。長鏈要仔細聽；重開鏈最好放在自然的音樂轉折點。
- **Turbo LoRA 與 Spectrum 都會吃掉音訊** — 8 步 Turbo LoRA 省掉的最後幾步正是細音訊。Spectrum 更會誤判固定行。兩樣一起用是疊加。**長片接續建議先關掉 Spectrum**；LoRA 因 10GB 顯存的 8 步速度是必要的，但要知道它讓音訊偏厚、畫面偏軟。
- **分辨率不能在鏈中改動** — latent 不能 resize，節點會直接拒絕。要改解析度得從那一鏡重新生成、或重開一條新鏈。
- **32 kHz 音訊** — H3 輸出是 32 kHz 不是 48 kHz。任何 re-mux / 合併腳本都要從檔案讀取真實取樣率，硬編 48000 會把長片尾巴變空白。
- 節點自測：`ComfyUI-H3-Motion-Context/tests/` 下有 `seam_probe.py`（接點是不是真接續）、`level_step.py`（音量／底噪有沒有跳）、`freeze_detect.py`（持住鏡頭是不是變成靜圖），可對自己的產物量測。

若要退回**穩定的尾幀接續**（不用 AV latent、每鏡原生音訊、無接續劣化但接點較硬），設環境變數：
`MINIMAX_H3_LONG_CONTINUITY=off`（任何非 `motion_context`/`motion`/`experimental` 的值都算 off），再重啟 Bot。
