...
# Force single GPU (cuda:0) to avoid dual-GPU issues with DynamicVRAM.
# Override with MINIMAX_COMFY_CUDA_VISIBLE_DEVICES (e.g. "0,1") if needed.
$env:CUDA_VISIBLE_DEVICES = if ($env:MINIMAX_COMFY_CUDA_VISIBLE_DEVICES) {
    $env:MINIMAX_COMFY_CUDA_VISIBLE_DEVICES
} else {
    '0'
}
