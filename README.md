...
輸入 `/start` 或 `/menu` 後，主畫面只保留「生成影片、提示詞／素材、生成設定、當前任務、ComfyUI／系統、歷史／長片／排隊」等核心按鈕；點入子頁面後再調整模式、片長、解析度、steps 和任務控制，手機上不用再向上翻一大串按鈕。解析度現在包括 1152×640、1280×736、1344×768 三個實驗檔位；在 10GB 顯存上較容易 OOM，建議先用 448×256 至 736×416。設定會保存到 Windows 使用者資料夾，下次啟動會讀回；提示詞和最近一張圖片也可以保留。
